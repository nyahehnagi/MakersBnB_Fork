...
describe BnbDate do
  it 'creates a bnb date' do
    bnb_date = BnbDate.create(bnbdate: '23-02-2022')
    persisted_date = BnbDate.find(date_id: bnb_date.date_id)

    expect(persisted_date.bnbdate).to eq bnb_date.bnbdate
  end
  ...
  it 'returns dates that have not been used yet' do
    ...
    date_used = BnbDate.create(bnbdate: '23-02-2022')
    date_not_used = BnbDate.create(bnbdate: '24-02-2022')
    ...
    expect(usable_dates.first.bnbdate).to eq date_not_used.bnbdate
  end
end
